...
- **Tocar:** `Panel.tsx` (selección persistente con checkboxes + barra flotante de acciones; el handler `correrLote` vive aquí), `lib/loteAcciones.ts` (helpers `ejecutarLote`/`resumenLote`/`notaReprogramacion`/`clasificarError`), modales de entrada, validación y feedback, invalidación de queries al terminar.
