...
| 2 | 2026-07-10 | Stakeholder solicita reorganizar la UI: sidebar de navegación con sección "Administración" visible solo para Dirección (Checklist, Usuarios), conservando el bloque de usuario en la topbar | Mejora | Shell con sidebar morado profundo (grupos General/Administración, activo lima, off-canvas en móvil); topbar conserva título/fecha/usuario/Salir; rutas y guards sin cambios. | ✅ Aplicado |
| 3 | 2026-07-14 | Recordatorios es operación interna → debe verse solo desde Administración (Dirección/Coordinación); falta una vista personal donde cada quien vea sus acuerdos designados (responsable o corresponsable) | Mejora | `/recordatorios` pasa al bloque Administración (guard de ruta, solo front); nace `/mis-acuerdos` en General con badge de participación y filtro `mios=1` en `GET /acuerdos` (ADR-013, doc 05 v1.8). | ✅ Aplicado |
